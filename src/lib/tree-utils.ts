...
export const buildTree = (people: Person[], relationships: any[]): TreeNode[] => {
  ...
  const constructNode = (personId: string, level: number, colorIndex: number): TreeNode | null => {
    ...
    // Children can come from the person, their spouses, or their siblings (if data is messy)
    const parentGroupIds = [personId, ...spouseIds, ...siblingIds];
  ...
  people.forEach(p => {
    if (!visitedIslands.has(p.id)) {
      ...
      // Find the best root for this island (someone with no parents)
      const islandRoots = island
        .filter(id => !parentsOf.has(id))
        .sort((a, b) => levels[a] - levels[b]);

      const rootId = islandRoots.length > 0 ? islandRoots[0] : island[0];
      const node = constructNode(rootId, levels[rootId], roots.length);
