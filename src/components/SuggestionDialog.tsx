...
const FIELD_CONFIG: Record<string, { label: string, icon: any, placeholder: string, type: 'input' | 'textarea' | 'select' | 'custom' }> = {
  ...
  nickname: { label: 'Nickname', icon: Tag, placeholder: 'e.g. "Bibi" or "NA"', type: 'input' },
  ...
  education: { label: 'Education', icon: GraduationCap, placeholder: '', type: 'custom' },
  military_service: { label: 'Military Service', icon: Shield, placeholder: 'Branch, rank, or "NA" if they did not serve.', type: 'textarea' },
      ...
      <DialogContent className="sm:max-w-md rounded-[2.5rem] border-none bg-stone-50 p-10 max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription className="text-stone-500 text-lg">
            {isEducation 
              ? `Manage the educational history for ${person.name.split(' ')[0]}.`
