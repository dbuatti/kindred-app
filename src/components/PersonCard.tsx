  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row">
        <div className="relative w-full md:w-40 aspect-[4/5] md:aspect-auto overflow-hidden shrink-0">
          {person.photoUrl ? (
            <img 
              src={person.photoUrl} 
              alt={person.name}
            ...
            <div className="flex items-start justify-between">
              <div className="space-y-0.5">
                ...
                <div className="flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[10px] text-stone-400 font-medium uppercase tracking-widest">
                  ...
                  {person.birthPlace && (
                    <span className="flex items-center gap-1">
                      <MapPin className="w-2.5 h-2.5" /> {person.birthPlace}
                    </span>
                  )}
                </div>
              </div>
              <button 
                onClick={handleShare}
                className="p-1.5 rounded-full hover:bg-stone-50 text-stone-300 hover:text-amber-600 transition-colors"
              >
          ...
          <div className="flex items-center justify-between pt-4 border-t border-stone-50">
            <div className="flex flex-wrap gap-1.5">
              {person.personalityTags.slice(0, 2).map(tag => (
                <Badge 
                  key={tag} 
                  variant="secondary" 
                  className="bg-stone-100 text-stone-500 hover:bg-stone-200 border-none rounded-full px-2 py-0 text-[9px] font-bold uppercase tracking-tighter"
                >
                  {tag}
                </Badge>
              ))}
            ...
            <div className="flex items-center gap-2 text-stone-300">
              <span className="flex items-center gap-1 text-[9px] font-bold">
                <MessageSquare className="w-2.5 h-2.5" /> {person.memories.length}
              </span>
            </div>
          </div>
        </div>
