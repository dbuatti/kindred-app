  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex items-center gap-3 overflow-x-auto pb-2 no-scrollbar">
        <div className="flex items-center gap-2 px-3 py-2 bg-stone-100 rounded-full text-stone-400 shrink-0">
          <Filter className="w-3 h-3" />
          <span className="text-[10px] font-bold uppercase tracking-widest">Filter</span>
        </div>
        {[
          { id: 'all', label: 'All Stories', icon: MessageSquare },
          { id: 'voice', label: 'Voice Notes', icon: Mic },
          { id: 'photo', label: 'Photos', icon: Camera },
          { id: 'text', label: 'Written', icon: MessageSquare }
        ].map((item) => (
          <button
            key={item.id}
      ...
      {allMemories.length === 0 ? (
        <div className="text-center py-24 space-y-6 bg-white rounded-[3rem] border-4 border-dashed border-stone-100">
          ...
          <div className="space-y-2">
            <p className="text-stone-800 font-serif text-2xl italic">No {filter !== 'all' ? filter : ''} stories found...</p>
            <p className="text-stone-400 text-lg max-w-md mx-auto leading-relaxed">
              {filter === 'all' 
                ...
                : `Be the first to share a ${filter} memory with the family.`}
            </p>
          </div>
        </div>
      ) : (
